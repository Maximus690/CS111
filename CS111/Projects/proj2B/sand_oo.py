...
def add_sand(grid, x, y):
    ...
    """
    if not grid.in_bounds(x, y) or grid.get(x, y) is not None:
        return  # Exit if the position is out of bounds or not empty
...
def remove_sand(grid, x, y):
    ...
    """
    sand_object = grid.get(x, y)
    if not isinstance(sand_object, Sand):
        return  # Exit if there is no sand particle at the specified position

    grid.set(x, y, None)
    all_sand.remove(sand_object)
