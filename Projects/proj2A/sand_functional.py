...
def is_move_ok(grid, x_from, y_from, x_to, y_to):
    ...
    if not grid.in_bounds(x_to, y_to):
        return False
    if grid.get(x_to, y_to) is not None:
        return False
    if x_from != x_to:
        return grid.get(x_to, y_from) is None
    return True
...
def do_move(grid, x_from, y_from, x_to, y_to):
    ...
    """
    grid_copy = grid.copy()
    sand_value = grid_copy.get(x_from, y_from)
    grid_copy.set(x_from, y_from, None)
    grid_copy.set(x_to, y_to, sand_value)
    return grid_copy
...
def do_gravity(grid, x, y):
    ...
    sand_value = grid.get(x, y)

    if sand_value != "s":
        return grid
    if is_move_ok(grid, x, y, x, y + 1):     # This is for trying to move down
        grid = do_move(grid, x, y, x, y + 1)
    elif is_move_ok(grid, x, y, x - 1, y + 1):  # This is for trying to move down-left
        grid = do_move(grid, x, y, x - 1, y + 1)
    elif is_move_ok(grid, x, y, x + 1, y + 1):  # This is for trying to move down-right
        grid = do_move(grid, x, y, x + 1, y + 1)
